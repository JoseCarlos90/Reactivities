    ...
    public class List
    {
        ...
        public class Handler : IRequestHandler<Query, ActivitiesEnvelope>
        {
                 ...
                 CancellationToken cancellationToken)
            {
                ...
                {
                    querable = querable.Where(x => x.UserActivities.Any(a => a.AppUser.UserName == _userAccesor.GetCurrentUsername()));
                }

                if (request.IsHost && !request.IsGoing)
                {
                    querable = querable.Where(x => x.UserActivities.Any(a => a.AppUser.UserName == _userAccesor.GetCurrentUsername() && a.IsHost));
                }
